<x-settings-layout>
    ...
    <div class="flex items-start justify-between mb-4">
        ...
        <a href="{{ route('halls.offers.create', session('hall')->id) }}" class="inline-block py-1 px-6 rounded-sm bg-slate-900 text-slate-300 text-sm hover:bg-slate-800 transition duration-150 ease-in-out">
            {{ __('actions.add.page')}}
        </a>
    </div>
        ...
        @foreach ($offers as $offer)
            <tr>
                ...
                <td class="px-6 py-3 text-right text-sm space-s-1 flex items-center justify-end">
                    ...
                    <!-- begin::Delete -->
                    <x-actions.delete :action="route('halls.offers.destroy', ['hall' => session('hall')->id, 'offer' => $offer->id])" />
                    <!-- end::Delete -->
                </td>
                <!-- end::Actions -->
            </tr>
