<x-app-layout>
    ...
    <form action="{{ route('halls.offers.update', ['hall' => session('hall')->id, 'offer' => $offer->id]) }}" method="POST" class="space-y-4">
        @csrf
        @method('PATCH')
        ...
        <div class="grid grid-cols-2 pt-8">
            <div class="col-span-1 flex items-center justify-between">
                ...
                <x-actions.back href="{{ route('halls.offers.index', ['hall' => session('hall')->id, 'offer' => $offer->id]) }}">
                    {{ __('actions.back')}}
                </x-actions.back>
            </div>
        </div>
