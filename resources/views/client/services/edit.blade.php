<x-app-layout>
    ...
    <form action="{{ route('halls.services.update', ['hall' => session('hall')->id, 'service' => $service->id]) }}" method="POST" class="space-y-4 pb-16">
        @csrf
        @method('PATCH')
        ...
        <div class="grid grid-cols-2 pt-4">
            <div class="col-span-1 flex items-center justify-between">
                ...
                <x-actions.back href="{{ route('halls.services.index', session('hall')->id) }}" />
            </div>
        </div>
        <!-- end::Form Button -->
    </form>
