<x-app-layout>
    ...
    <form action="{{ route('halls.booking-times.update', ['hall' => session('hall')->id, 'booking_time' => $bookingTime->id]) }}" method="POST" class="space-y-4">
        @csrf
        @method('PATCH')
        ...
        <div class="grid grid-cols-2 pt-8">
            <div class="col-span-2 max-w-[560px] flex items-center justify-between">
                ...
                <x-actions.back href="{{ route('halls.booking-times.index', session('hall')->id) }}">
                    {{ __('actions.back')}}
                </x-actions.back>
            </div>
        </div>
